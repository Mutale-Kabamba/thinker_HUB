                ...
                <div class="hub-card" style="padding:0.75rem 1rem;margin-bottom:0.65rem;">
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:0.5rem;">
                        <div style="min-width:0;flex:1;">
                            <p style="margin:0;font-weight:600;color:var(--hub-ink);font-size:0.88rem;">{{ $assignment['name'] }}</p>
                            <p style="margin:0.15rem 0 0;font-size:0.74rem;color:var(--hub-muted);">{{ $assignment['course'] }} &middot; {{ $assignment['scope'] }}</p>
                        </div>
                        <span class="hub-chip {{ in_array($assignment['status'], ['Graded','Checked']) ? 'hub-chip-green' : ($assignment['status'] === 'Submitted' ? 'hub-chip-blue' : 'hub-chip-amber') }}" style="font-size:0.7rem;flex-shrink:0;">{{ $assignment['status'] }}</span>
                    </div>

                    {{-- Meta: due date + grade --}}
                    <div style="display:flex;gap:1rem;margin-top:0.45rem;font-size:0.78rem;">
                        <span style="color:var(--hub-muted);">Due: {{ $assignment['due'] }}</span>
                        <span style="font-weight:700;color:{{ $assignment['grade'] !== null ? '#15803d' : 'var(--hub-muted)' }};">
                            Grade: {{ $assignment['grade'] !== null ? $assignment['grade'] . '%' : '-' }}
                        </span>
                    </div>
                    ...
                    {{-- Expandable Details --}}
                    <div x-show="expanded === {{ $assignment['id'] }} && panel === 'details'" x-cloak x-collapse>
                        <div style="margin-top:0.6rem;padding:0.75rem;background:var(--hub-surface);border-radius:8px;border:1px solid var(--hub-border);">
                            ...
                            @if (!empty($assignment['description']))
                                <div style="padding:0.5rem 0.65rem;background:#fff;border:1px solid var(--hub-border);border-radius:8px;margin-bottom:0.5rem;">
                                    ...
                                    <p style="margin:0.2rem 0 0;font-size:0.8rem;color:var(--hub-muted);white-space:pre-line;">{{ $assignment['description'] }}</p>
                                </div>
                            @endif

                            @if (in_array($assignment['status'], ['Graded', 'Checked']))
                                <div style="padding:0.5rem 0.65rem;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;margin-bottom:0.5rem;">
                                    <div style="display:flex;gap:1rem;align-items:center;flex-wrap:wrap;">
                                        @if ($assignment['grade'] !== null)
                                            <div>
                                                <p style="margin:0;font-size:0.68rem;font-weight:600;color:#166534;text-transform:uppercase;">Grade</p>
                                                <p style="margin:0;font-size:1.1rem;font-weight:800;color:#15803d;">{{ $assignment['grade'] }}%</p>
                                            </div>
                                        @endif
                                        <div>
                                            <p style="margin:0;font-size:0.68rem;font-weight:600;color:#166534;text-transform:uppercase;">Status</p>
                            ...
                            <p style="margin:0;font-size:0.72rem;color:var(--hub-muted);">Submitted: {{ $assignment['submitted_at'] ?: 'Not yet' }}</p>
                        </div>
                    </div>

                    {{-- Expandable Submit --}}
                    <div x-show="expanded === {{ $assignment['id'] }} && panel === 'submit'" x-cloak x-collapse>
                        <div style="margin-top:0.6rem;padding:0.75rem;background:var(--hub-surface);border-radius:8px;border:1px solid var(--hub-border);">
                            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.5rem;">
                                <h4 style="margin:0;font-size:0.85rem;font-weight:700;color:var(--hub-ink);">
                                    {{ $assignment['status'] === 'Not submitted' ? 'Submit Work' : 'Resubmit / Update' }}
                                </h4>
                                <button @click="expanded = null; panel = null;" style="background:none;border:none;cursor:pointer;color:var(--hub-muted);font-size:1.1rem;line-height:1;" title="Close">&times;</button>
                            </div>
                            <div style="display:flex;flex-direction:column;gap:0.45rem;">
