            ...
            @forelse ($materials as $material)
                <div class="hub-card" style="padding:0.75rem 1rem;margin-bottom:0.65rem;">
                    ...
                    <div style="display:flex;gap:0.35rem;margin-top:0.55rem;flex-wrap:wrap;">
                        ...
                        @endif
                        @if (!empty($material['link_url']))
                            <a href="{{ $material['link_url'] }}" target="_blank" rel="noopener" wire:click="recordView({{ $material['id'] }})" style="background:none;border:1px solid var(--hub-border);border-radius:6px;padding:0.3rem 0.65rem;font-size:0.75rem;cursor:pointer;color:#0d9488;font-weight:600;text-decoration:none;">Open Link</a>
                        @endif
                        @if (!empty($material['video_url']))
